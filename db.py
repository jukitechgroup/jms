...
def modbus_setup(SlaveNumber, baudrate):
    instrument = minimalmodbus.Instrument('/dev/ttyAMA0', SlaveNumber) # port name, slave address (in decimal)
...
def serial():
 ...
 # ------------ databaseen laitto functio ------

def update_db():
    con = lite.connect('/home/maintenance/jms/jms/data/measure.db') # Luodaan sqlite yhteys
    
    def db_1():   										
        value = sensor1_data												
        aika = strftime("%Y-%m-%d %H:%M:%S")						# Aika tietokantaa varten 
        with con:																			# Yhdistetään tietokantaan
            cur = con.cursor()																#
            cur.execute("CREATE TABLE IF NOT EXISTS measure1(aika TEXT, value INT)")		# Luodaan tietokantaan taulukko jos ei jo ole 
            cur.execute("INSERT INTO measure1 (aika, value) VALUES(?,?)", (aika, value))	# Laitetaan tiedot tietokantaan
